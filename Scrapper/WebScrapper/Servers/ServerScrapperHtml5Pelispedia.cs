    ...
    public class ServerScrapperHtml5Pelispedia : IServerScrapper
    {
        ...
        public override bool scrappear (string url, ref Sources serverLinks, ref string error)
        {
            ...
            if (0 == error.Length)
            {
                ...
                {
                    if (!HttpManager.requestGet(linkVideo, null, ref buffer, ref error))
                        break;
        ...
        bool obtenerUrlVideo (string linkVideo, string token, ref string urlVideo, ref string error)
        {
            try
            {
                ...
                headers.Add(new HttpHeader("ContentType", "application/x-www-form-urlencoded; charset=UTF-8"));

                if (!HttpManager.requestPost(url, headers, request, ref buffer, ref error))
